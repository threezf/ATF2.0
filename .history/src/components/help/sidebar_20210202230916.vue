<template>
    ...
    <nav class="menu menu-vertical el-menu" :class="{ 'menu-collapse': isCollapse, 'menu-expand':!isCollapse}" >
            ...
            <div v-if="showByCategory">
                <div class="category" v-for="(values, key) in categoryMap" :key="key">
                    ...
                    <div v-if="values.length != 0" class="content">
                        <div class="item" v-for="v in values" :key="v.id"><a @click="goTop('#project-' + v.id)" data-scroll>{{v.name}}</a>
                        </div>
