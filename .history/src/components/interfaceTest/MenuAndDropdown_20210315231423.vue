<template>
    <div>
        <template v-for="menu in menuData">
            <el-submenu :key="menu.index" :index="menu.index" style="text-align:center;" v-if="menu.children" :disabled="visible">
                <template slot="title">
                    <span slot="title" :class="isCollapse? 'isisCollapse-span':'open-span'">{{menu.name}}</span>
                    <span>
                <el-dropdown  trigger="click" @click.native="visible=true">
                        <span class="el-dropdown-link" >
                            <i class="el-icon-arrow-down el-icon-more"></i>
                        </span>
                        <el-dropdown-menu slot="dropdown">
                            <el-dropdown-item icon="el-icon-circle-plus-outline">新建子分组</el-dropdown-item>
