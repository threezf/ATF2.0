...
<div>
    <el-tabs v-model="bodyShow" type="border-card">
        <el-tab-pane label="请求头部" name="first">
            <el-table :data="apiMsgData.header"
                        size="mini"
        ...
        <el-tab-pane label="请求参数" name="third">
            <el-table :data="apiMsgData.params" size="mini" stripe :show-header="true"
                        border
                        class="h-b-e-a-style"
                        :row-style="{'background-color': 'rgb(250, 250, 250)'}">
                <el-table-column property="key" label="Key" header-align="center" minWidth="100">
        ...
        <el-tab-pane label="权限校验" name="fourth">
            <el-row>
                ...
                <el-select size="small" v-model="apiMsgData.selectedAuthorization">
...
export default {
    ...
    methods: {
            ...
            if (response['status'] === 0) {
                this.$confirm('服务器已存在相同名字文件，是否覆盖?', '提示', {
                    ...
                    this.$axios.post('/api/upload', form).then((response) => {
                            ...
                            this.apiMsgData.body[this.temp_num]['value'] = response.data['data'];
                        }
                    );
                }).catch(() => {
                ...
                this.apiMsgData.body[this.temp_num]['value'] = response['data'];
            }
        ...
        delTableRow(type, i) {
            if (type === 'body') {
                this.apiMsgData.body.splice(i, 1);
            } else if (type === 'header') {
        ...
        addTableRow(type) {
            if (type === 'body') {
                this.apiMsgData.body.push({key: null, value: null, param_type: 'string', remark: null});
            } else if (type === 'header') {
                this.apiMsgData.header.push({key: null, value: null});
            } else if (type === 'params') {
                this.apiMsgData.params.push({key: null, value: null, remark: null});
            }
        },
        initApiMsgData() {
                console.log("测试")
                ...
                this.apiMsgData.body = Array();
                this.apiMsgData.jsonVariable = '';
                this.apiMsgData.params = Array();
            },
    },
    created() {
        this.initApiMsgData()
    ...
    watch: {
        ...
        monitorVariable: {
            handler: function () {
                if (this.apiMsgData.body.length === 0) {
                    this.addTableRow('body')
                }
                if (this.apiMsgData.body[this.apiMsgData.body.length - 1]['key'] || this.apiMsgData.body[this.apiMsgData.body.length - 1]['value']) {
                    this.addTableRow('body')
                }
            },
            deep: true
        ...
        monitorExtract: {
            handler: function () {
                if (this.apiMsgData.params.length === 0) {
                    this.addTableRow('params')
                }
                if (this.apiMsgData.params[this.apiMsgData.params.length - 1]['key'] || this.apiMsgData.params[this.apiMsgData.params.length - 1]['value']) {
                    this.addTableRow('params')
                }
            },
            deep: true
