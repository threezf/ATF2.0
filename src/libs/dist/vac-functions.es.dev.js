...
var Vac = Vac || {}; //检测用户登录状态，添加 更改操作需要有登陆权限
...
function loginDetect() {
  return new Promise(function (resolve, reject) {
    ...
    if (sessionId !== null && sessionId !== "") {
      ...
      client.setRequestHeader("Accept", "application/json"); // console.log(sessionId);
...
Vac.isHasClass = function (element, className) {
  ...
  var pattern = new RegExp('\\b' + className + '\\b', 'g'); // String.search() 方法返回的是第一个匹配项的index，没有匹配项时返回-1
...
Vac.startDrag = function (triggerElement, targetElement) {
  ...
  triggerElement.addEventListener('mousedown', function (event) {
    var originMouseX, originMouseY, moveX, moveY; // 保存鼠标点下时的初始值
    ...
    function mouseMove(event) {
      ...
      targetElement.style.top = +moveY + +targetElement.offsetTop + 'px'; // 防止选取文字
...
Vac.getOffsetTo = function (child, ancestor) {
  ...
  var current = child.offsetParent;

  while (current !== ancestor) {
    offsetTop = offsetTop + current.offsetTop + parseFloat(window.getComputedStyle(current).borderTopWidth);
    offsetLeft = offsetLeft + current.offsetLeft + parseFloat(window.getComputedStyle(current).borderLeftWidth);
...
Vac.ajaxDownload = function (url, data, method) {
  // 获得url和data
  if (url && data) {
...
Vac.formValidation.prototype._valid = function (inputId, iconId, tooltipId, rules) {
  ...
  var tooltip = document.querySelector('#' + tooltipId); // use Approvejs to validate the value

  var results = approve.value(input.value, rules);

  if (results.approved) {
    (0, _jquery["default"])(icon).addClass('icon-ok correct').removeClass('hide icon-warning-sign error');
    (0, _jquery["default"])(tooltip).removeClass('show').addClass('hide');
    return true;
  } else {
    (0, _jquery["default"])(icon).addClass('icon-warning-sign error show').removeClass('hide correct icon-ok ');
    (0, _jquery["default"])(tooltip).addClass('show').removeClass('hide');
    tooltip.innerHTML = '';
    results.each(function (error) {
...
Vac.formValidation.prototype.validAll = function () {
  ...
  return this.inputs.every(function (item) {
    return _this2._valid(item.inputId, item.iconId, item.tooltipId, item.rules);
  });
};
