<template>
  <div class="page-inner">
    ...
    <div class="ele-container">
      <div :class="'ele-left ' + (saidBarShow ? '' : 'narrow-ele-left')">
        ...
        <div class="treeHideIcon" v-if="saidBarShow">
          <el-button size="mini" @click="saidBarShow = !saidBarShow"
    ...
    <el-dialog title="编辑数据" :visible.sync="editDataFlag" width="50%">
                    ...
                    <el-button
                      @click="
                        scope.row.parameters &&
                          scope.row.parameters.forEach((v) => {
                            v.newValue = v.Value;
                          });
      ...
      <div>
        ...
        >
          ...
          <div slot="empty">暂无数据校验</div>
        </el-table>
      </div>
      <el-row type="flex" justify="center">
        <el-col :span="6">
          <el-button type="primary" @click="editDataFlag = false"
            >取消</el-button
          >
    ...
    <el-dialog
      title="用例筛选"
      :visible.sync="searchTemplateDailog"
      width="30%"
    >
...
export default {
  ...
  methods: {
    ...
    changeMethod(item, flag) {
      ...
      item.parameters = theFunction.arguments
        ? JSON.parse(theFunction.arguments).map((item) => ({
            Name: item.name,
            Value: "",
            newValue: "",
          }))
    ...
    deleteTemplateInfo(flag) {
      if (flag == 1) {
        ...
        this.beforeOperationRows = this.beforeOperationRows.filter(
          (item) => !item.selected
        );
      }
      if (flag == 2) {
        this.afterOperationRows = this.beforeOperationRows.filter(
          (item) => !item.selected
        );
      }
      if (flag == 3) {
        this.dataOperationRows = this.beforeOperationRows.filter(
          (item) => !item.selected
        );
      }
    },
    eleUp(type) {
      if (type == 1) {
        ...
        for (let i = 0; i < this.beforeOperationRows.length; i++) {
          if (
            this.multipleSelection1.some(
              (v) => v.id === this.beforeOperationRows[i].id
            )
          ) {
            if (flag2) {
        ...
        for (let i = 0; i < this.afterOperationRows.length; i++) {
          if (
            this.multipleSelection2.some(
              (v) => v.id === this.afterOperationRows[i].id
            )
          ) {
            if (flag3) {
        ...
        for (let i = 0; i < this.dataOperationRows.length; i++) {
          if (
            this.multipleSelection2.some(
              (v) => v.id === this.dataOperationRows[i].id
            )
          ) {
            if (flag3) {
    ...
    editData(row, column) {
      ...
      if (cellData) {
        ...
        if (cellData.includes("@before")) {
          var beforeStr = cellData.slice(
            cellData.indexOf("@before\n") + 7,
            cellData.indexOf("@value")
          );
        ...
        if (cellData.includes("@after")) {
          var afterStr = cellData.slice(
            cellData.indexOf("@after\n") + 6,
            cellData.indexOf("@display") === -1
              ? undefined
              : cellData.indexOf("@display")
          );
        ...
        if (cellData.includes("@value")) {
          var endIndex = cellData.includes("@after")
            ? cellData.indexOf("@after")
            : cellData.length;
          valueStr = cellData
            .slice(cellData.indexOf("@value") + 6, endIndex)
            .replace(/^\s$/g, "");
        } else {
          valueStr = cellData;
        }
      ...
      if (strArray.length) {
        for (let i = 0; i < length; i++) {
          ...
          if (str.includes("UI(")) {
            ...
            var paraArr = script[2]
              .slice(script[2].indexOf("(") + 1, -1)
              .split(",");
            var parameters = [];
    ...
    dataTemplate() {
      ...
      Request({
        ...
        let url =
          "http://10.101.167.184:8080/" + res.filePath.split("/webapps/")[1];
      ...
      if (this.beforeOperationRows.length > 0) {
        ...
        for (let i = 0; i < this.beforeOperationRows.length; i++) {
          if (this.beforeOperationRows[i].parameters.length !== 0) {
            ...
            for (
              let j = 1;
              j < this.beforeOperationRows[i].parameters.length;
              j++
            ) {
      ...
      if (this.afterOperationRows.length > 0) {
        ...
        for (let i = 0; i < this.afterOperationRows.length; i++) {
          if (this.afterOperationRows[i].parameters.length !== 0) {
            ...
            for (
              let j = 1;
              j < this.afterOperationRows[i].parameters.length;
              j++
            ) {
      ...
      if (this.dataOperationRows.length > 0) {
        ...
        for (let i = 0; i < this.dataOperationRows.length; i++) {
          if (this.dataOperationRows[i].parameters.length !== 0) {
            ...
            for (
              let j = 1;
              j < this.dataOperationRows[i].parameters.length;
              j++
            ) {
