	...
	<div class="page-outer">
		<div class="page-inner">
			<div class='ele-container'>
				...
				<el-table v-loading='templateLoading' border ref="singleTable" :data="templateList" highlight-current-row
									row-key="id" @row-click='chooseTemplate' style="width: 100%">
				...
				<div class="templatInfo">
					...
					<div>
						...
						<el-table border ref="multipleTable" v-loading='templateLoading' :data="templateInfo" tooltip-effect="dark"
											style="width: 100%" @selection-change="handleSelectionChange" row-key="name" class="sortable">
							...
							<el-table-column label="方法" width="180">
								<template slot-scope="scope">
									<el-select v-model="scope.row.methodName" placeholder="请选择" @change="changeMethod(scope.row)">
										<el-option v-for="item in methods[scope.row.elementWidget]" :key="item.id" :label="item.name"
															 :value="item.name">
										</el-option>
									</el-select>
								</template>
								...
								<template slot-scope="scope">
									...
									<div v-else>
										...
										<el-row v-for='item in scope.row.arguments' :key='item.name'>
											...
											<el-col :span="10">
												<el-input size="mini" @dragenter.stop.prevent="return false"
																	@dragover.stop.prevent="return false" v-model="item.newvalue"></el-input>
											</el-col>
										</el-row>
										<el-row>
											<el-col :span="5">
		...
		<el-dialog title="添加多项" :visible.sync="addItemShow" width="30%">
			<uiEleFunTree @closeDialog="addItemShow = false" @throwTreeInfo="addTreeInfo" :multiselection='true'
										:transId='transId.toString()' :autId='autId.toString()'>
			</uiEleFunTree>
		</el-dialog>
	</div>
</template>
