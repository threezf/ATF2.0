<template>
	<div class="class-environment">
		...
		<el-dialog :title="getTitle" :visible.sync="environmentVisible" width="50vw" :append-to-body="true">
			<el-form :model="newEnvironmentData" ref="newEnvironmentData" label-width="80px" label-position='top'>
				<el-row>
					<el-col :span="24">
						<el-form-item label="环境名称" prop="name" class="change-label-calss">
							<el-input v-model="newEnvironmentData.name" size="small" placeholder="请输入环境名称" style="width:100%"/>
						</el-form-item>
					</el-col>
				</el-row>
				<el-row>
					<el-col :span="24">
						<el-form-item label="环境备注" prop="remark" class="change-label-calss">
								<el-input v-model="newEnvironmentData.remark" size="small"style="width:100%">
								</el-input>
						</el-form-item>
					</el-col>
				</el-row>
				<el-row>
					<el-col :span="24">
						<el-form-item label="Host" prop="url" class="change-label-calss">
							<span>选择当前环境时，会自动给没有Host的API加上统一的Host前缀，如果API地址中已有Host信息，则不会再添加环境的Host前缀。</span>
								<el-input v-model="newEnvironmentData.url" size="small" style="width:100%">
								</el-input>
						</el-form-item>
					</el-col>
				</el-row>
				...
				<el-row>
					<TestTabs ref="testTabs" :body="newEnvironmentData.bodyContent" :header="newEnvironmentData.header"
			...
			<el-form :model="copyEnvironmentData" label-position='top'>
				<el-row>
						<el-form-item label="环境名称" prop="interfaceName" class="change-label-calss">
								<el-input v-model="copyEnvironmentData.name" size="small" placeholder="环境名称" style="width:100%">
								</el-input>
						</el-form-item>
				</el-row>
				<el-row class="buttons_row">
...
export default {
	...
	data() {
		return {
			...
			resultData:[{
				...
				name:'测试环境',
				remark:'配置测试环境',
				url:'http://140.143.16.21:9090/'
			},{
				id:'环境2',
				name:'生产环境',
				remark:'配置生产环境',
				url:'http://140.143.16.21:8080/'
			}],
	...
	methods:{
		...
		copyEnvironmentButton(row){
				...
				this.copyEnvironmentData.name = '副本-'+row.name
