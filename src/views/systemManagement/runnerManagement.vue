...
<page>
    <el-header>
        ...
        <el-button size="small" type="primary" icon="el-icon-download" @click='downloadRunner'>
            执行机安装包下载
        </el-button>
    ...
    <el-table stripe border :data="tableData" class='table'>
        ...
        <el-table-column label="IP" property="runnerIp" width="180" />
        <el-table-column label="端口号" property='runnerPort' width="120" />
        <el-table-column label="描述" property="runnerName" width="200" />
        <el-table-column label="日志查看">
            <template slot-scope="scope">
                <el-button size="mini" @click="handleDetail(scope.$index, scope.row)">日志查看
                </el-button>
            </template>
        </el-table-column>
    </el-table>
    <el-card class="box-card" shadow="always" v-show='logVisible'>
        <div slot="header" class="clearfix">
            <span>执行机日志</span>
            <el-button style="float: right; padding: 3px 0" type="text" @click="logVisible=false">
                隐藏
            </el-button>
        </div>
        <div class="text item">
            <el-input id='textarea_id' type="textarea" placeholder="请输入内容" disabled v-model="log" :rows="15">
            </el-input>
        </div>
    </el-card>
