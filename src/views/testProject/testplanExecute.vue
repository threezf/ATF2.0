...
<div>
                    ...
                    <div class="main" style="padding: 15px; user-select: none; min-width: 100%">
                        ...
                        <div class="main-content" id="testround-main" style="padding: 15px 0;">
                            ...
                            <div id="">
                                ...
                                <div class="main-content2">
                                    <div class="case-list-wrapper list-wrapper" v-if="testCaseList && testCaseList.length" style="padding:10px 20px 10px 20px">
                                        ...
                                        <div class="case-list">
                                            <div class="check-all-case" v-if="testCaseList && testCaseList.length" style="margin: 10px 10px;">全选
                                                        ...
                                                        <div class="case-wrapper">
                                                            <div class="case" @click="changeCase(case_wrapper.caseId, 1)" @mousedown.stop="1">
                                                                <div class="case-header">
                                                                    ...
                                                                    <p>
                                                                        <a class="view-case" @click.stop="viewCase(case_wrapper.caseId, 'PE4')"><span>查看</span></a>
                                                                    </p>
                                                                </div>
                            ...
                            <header class="panel-heading" v-if="testSceneList.length">场景列表
                                <button class="hide-case-list btn btn-xs btn-info" @mousedown.stop="1" @click="saveScenesSort()">保存场景排序
                                </button>
                                <span class="exe-info">
                            ...
                            <div class="sortable_sceneslist" style="margin-top: 20px;">
                                <div class="scene-list-wrapper  -wrapper" style="padding:0px 20px 10px 20px" v-for="(scene,index) in testSceneList" :key='index'>
                                    ...
                                    <button class="hide-case-list btn btn-xs btn-info" @click="hideCaseList($event)" @mousedown.stop="1"><span>展开</span>&nbsp;&nbsp;<i class="icon-caret-right"></i>
                                    </button>
                                    <button class="hide-case-list btn btn-xs btn-info" @mousedown.stop="1" @click="saveSort(scene.sceneId)">保存排序
                                    </button>
                                    <!-- <div class="case-list"> -->
                                    <div class="case-list" style="display:none">
                                        <div class="check-all-case" v-if="scene.testCaseList && scene.testCaseList.length" style="padding: 2px 5px;margin: 10px 10px;" @mousedown.stop="1">全选用例
                                                    ...
                                                    <div class="case-wrapper">
                                                        <div class="case" @click="changeCase(scene.sceneId+'-'+case_wrapper.caseId, 3)" @mousedown.stop="1">
                                                            <div class="case-header">
                                                                ...
                                                                <input type="checkbox" :value="'sort'+'-'+scene.sceneId+'-'+case_wrapper.caseId" class="check-case check-case-inscene" v-model="selectedSceneCases" @change="checkFlowNode($event)">
                                                                <!--lrlrq -->
                                                                <p>
                                                                    <a class="view-case" @click.stop="viewCase(scene.sceneId, case_wrapper.caseId, 'PE6')"><span>查看</span></a>
                                                                </p>
                                                            </div>
                                                        </div>
                                                        <div class="runnerclass" :id="'runner' + '-' + scene.sceneId+'-'+case_wrapper.caseId">
                                                            <p></p>
                                                        </div>
                                                    </div>
                                                </template>
                                                        ...
                                                        <div class="case caseFlow" v-for="(item,index) in case_wrapper.flowNodes" :key="'case_wrapper2'+index" @mousedown.stop="1" @click="changeCase(scene.sceneId+ '-' + case_wrapper.caseId + '-' + item.flowNodeId, 3)">
                                                            <div style="display:flex;align-items: center;">
                                                                <div class="case-header case-header-Flow">
                                                                    ...
                                                                    <input type="checkbox" v-bind:value="scene.sceneId+ '-' + case_wrapper.caseId + '-' + item.flowNodeId" class="check-case check-case-inscene" @change="checkFlowNode($event)" v-model="selectedSceneCases">
                                                                    <p>
                                                                        <a class="view-case" @click.stop="viewCase(scene.sceneId, case_wrapper.caseId, 'PE6', item.flowNodeId)">查看
                                                                        </a>
                                                                    </p>
                                                                    <!-- <a class="view-case"
        ...
        <div class="col-lg-12 text-center">
            <div class="pagination-wrap" v-if="scenePage.totalPage!=0">
                ...
                <ul class="pagination">
                    ...
                    <li :class="scenePage.currentPage==1?'disabled':''"><a @click="turnToScenePage(scenePage.currentPage-1)" href="javascript:;">上一页</a></li>
                    <li>
                    ...
                    <li class="active"><a href="javascript:;" @click="turnToScenePage(scenePage.currentPage)" v-text="scenePage.currentPage">3</a></li>
                    <li>
                    ...
                    <li :class="scenePage.currentPage==scenePage.totalPage?'disabled':''"><a href="javascript:;" @click="turnToScenePage(scenePage.currentPage+1)">下一页</a>
                    </li>
                    <li :class="scenePage.currentPage==scenePage.totalPage?'disabled':''"><a href="javascript:;" @click="turnToScenePage(scenePage.totalPage)">尾页</a>
                    </li>
                </ul>
                <div class="go">
    ...
    <el-dialog title="测试计划管理" :visible.sync="testPlanDialog" width="50%">
        ...
        <el-row>
            <el-pagination @size-change="turnToPage(1)" @current-change="(val)=>{turnToPage(val)}" :current-page="page.currentPage" :page-sizes="[5, 10, 20, 50]" :page-size="page.pageSize" layout="total, sizes, prev, pager, next, jumper" :total="page.totalPage">
            </el-pagination>
        </el-row>
    </el-dialog>
    <!--新增测试轮次 -->
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-10">
                                <select class="form-control" v-model="addRoundRowData.recordmanagementflag">
                                    <option v-for="option in [{ value: 1, label: '仅保存一份' }, { value: 2, label: '保存多份' }]" :value="option.value" :key="option.value">
                                        {{option.label}}
                                    </option>
                                </select>
                            </div>
                ...
                <div class="form-group display" v-show="1 === editType">
                    ...
                    <div class="col-sm-10">
                        <select name="testRoundId" v-model="addRowData.testRoundId" style="width:80%;height: 34px;" filterable placeholder="请选择">
                            <option v-for="testRound in testRoundArray" :key="testRound.id" :label="testRound.name" :value="testRound.id">
                            </option>
                        </select>
                    </div>
                </div>
