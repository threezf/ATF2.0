...
<div>
            ...
            <section id="main-content">
                <section class="content-wrapper" style="margin-bottom: 50px">
                    <div class="main" style="padding: 15px; user-select: none; min-width: 100%">
                        ...
                        <div class="main-content" id="testround-main" style="padding: 15px 0px">
                            ...
                            <el-form class="testplan" label-width="80px">
                                <el-row :gutter="20">
                                    ...
                                    <el-col :span="2">
                                        <el-button style="margin:5px 10px" type="primary" size="mini" @click="testPlanManager()">测试计划管理</el-button>
                                    </el-col>
                                    <el-col :span="4">
                                    ...
                                    <el-col :span="5">
                                        <el-form-item label="分配模式">
                                            <el-select size="small" v-model="runnerExecuteType" placeholder="请选择">
                                    ...
                                    <el-col :span="5">
                                        <el-form-item label="执行机选择">
                                            <el-select size="small" v-model="runnerselected" placeholder="请选择执行机" :multiple="true">
                            ...
                            <div class="test-control container" style="margin: 15px 5px">
                                ...
                                <el-button type="primary" size="mini" @click="executeAll()" v-if="notTimerFlag"><i class="icon-play"></i> 批量执行</el-button>
                            ...
                            <div id="">
                                ...
                                <div class="main-content2">
                                    <div class="case-list-wrapper list-wrapper" v-if="testCaseList && testCaseList.length" style="padding: 10px 20px 10px 20px">
                                        <!-- <span class="case-list-title list-title">用例列表</span> -->
                                        <header class="panel-heading" style="margin: 0 -15px">
                                            用例列表
                                        </header>
                                        <button class="hide-case-list btn btn-xs btn-info" @click="hideCaseList($event)" style="padding: 3px 6px; margin: 5px 10px" @mousedown.stop="1">
                                            <span>收起</span>&nbsp;&nbsp;<i class="icon-caret-down"></i>
                                        </button>
                                        <div class="case-list">
                                            <div class="check-all-case" v-if="testCaseList && testCaseList.length" style="margin: 10px 10px">
                                                全选
                                  ...
                                  " @mousedown.stop="1">
                                    ...
                                    height: 20px;
                                  " @mousedown.stop="1">
                                                                <input type="checkbox" :value="case_wrapper.caseId" class="checkall" v-model="selectedCases" @change="checkallToggle($event)" @mousedown.stop="1" />
                                                            </div>
                                                        </div>
                                                        <div class="case-wrapper">
                                                            <div class="case" v-for="(item,
                                  index) in case_wrapper.flowNodes" :key="'case_wrapper' + index" @mousedown.stop="1" @click="changeCase(item.flowNodeId, 2)">
                                                                <div class="case-header">
                                                                    <img :src="exeImgs[0]" :id="'img' + '-' + item.flowNodeId" />
                                                                    <p class="width-limit" :title="item.casecode">
                                                                        {{ item.casecode }}
                                                                    </p>
                                                                    <input type="checkbox" v-bind:value="item.flowNodeId" class="check-case flow-node-incaselib" @change="checkChanged($event)" v-model="checkedFlowNodes" />
                                                                    <p>
                                                                        <a class="view-case" @click.stop="
                                          viewCase(case_wrapper.caseId, 'PE4')
                                        "><span>查看</span></a>
                            ...
                            <header class="panel-heading" v-if="testSceneList.length">
                                场景列表
                                <button class="hide-case-list btn btn-xs btn-info" @mousedown.stop="1" @click="saveScenesSort()">
                                    保存场景排序
                                </button>
                                <span class="exe-info">
                            ...
                            <div class="sortable_sceneslist" style="margin-top: 20px">
                                <div class="scene-list-wrapper -wrapper" style="padding: 0px 20px 10px 20px" v-for="(scene, index) in testSceneList" :key="index">
                                    ...
                                    <div class="checkbox-wrapper" style="display: inline-block; padding: 2px 5px" @mousedown.stop="1">
                                        <input type="checkbox" :value="scene.sceneId" class="checkscene" v-bind:name="scene.id" v-model="selectedScenes" />
                                        <!-- lrqlrq-->
                                    </div>
                                    <span class="scene-list-title list-title" style="top: 2px">{{ scene.sceneName }}</span>
                                    <button class="hide-case-list btn btn-xs btn-info" @click="hideCaseList($event)" @mousedown.stop="1">
                                        <span>展开</span>&nbsp;&nbsp;<i class="icon-caret-right"></i>
                                    </button>
                                    <button class="hide-case-list btn btn-xs btn-info" @mousedown.stop="1" @click="saveSort(scene.sceneId)">
                                        保存排序
                                    </button>
                                    <!-- <div class="case-list"> -->
                                    <div class="case-list" style="display: none">
                                        <div class="check-all-case" v-if="scene.testCaseList && scene.testCaseList.length" style="padding: 2px 5px; margin: 10px 10px" @mousedown.stop="1">
                                            全选用例
                                ...
                                " @mousedown.stop="1">
                                    ...
                                    '-' +
                                    case_wrapper.caseId
                                  " class="check-flownodes" v-bind:name="case_wrapper.caseId" v-model="selectedSceneCases" @change="checkAllFlowNodes($event)" />
                                                        </div>
                                                    </div>
                                                    <div class="case-wrapper">
                                                        <div class="case" @click="
                                  changeCase(
                                ...
                                " @mousedown.stop="1">
                                    ...
                                    " />
                                      ...
                                      '-' +
                                      case_wrapper.caseId
                                    " class="check-case check-case-inscene" v-model="selectedSceneCases" @change="checkFlowNode($event)" />
                                                                <!--lrlrq -->
                                                                <p>
                                                                    <a class="view-case" @click.stop="
                                        viewCase(
                                ...
                                " @mousedown.stop="1">
                                    ...
                                    '-' +
                                    case_wrapper.caseId
                                  " v-bind:name="case_wrapper.caseId" class="check-flownodes" @change="checkAllFlowNodes($event)" v-model="selectedSceneCases" />
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="case-wrapper">
                                                        <div class="case caseFlow" v-for="(item, index) in case_wrapper.flowNodes" :key="'case_wrapper2' + index" @mousedown.stop="1" @click="
                                  changeCase(
                                    scene.sceneId +
                                      '-' +
                                      case_wrapper.caseId +
                                ...
                                ">
                                        ...
                                        item.flowNodeId
                                      " />
                                                                    <p class="width-limit" :title="item.flowNodeCode">
                                                                        {{ item.flowNodeCode }}
                                                                    </p>
                                                                    <input type="checkbox" v-bind:value="
                                        scene.sceneId +
                                        '-' +
                                        case_wrapper.caseId +
                                        '-' +
    ...
    <el-dialog title="新增场景" :visible.sync="senceDialog" width="50%">
            ...
            <table class="table table-bordered">
                ...
                <tbody>
                    <tr v-for="(scene, index) in allscenes" :key="'scene' + index">
                        ...
                        <td v-else>
                            <input size="small" type="checkbox" v-model="selectedScene" v-bind:value="scene.id" />
                        </td>
        ...
        <div class="col-lg-12 text-center">
            <div class="pagination-wrap" v-if="scenePage.totalPage != 0">
                <div class="num">
                    <select size="1" name="sample_1_length" aria-controls="sample_1" class="form-control" v-model="scenePage.pageSize" id="pageSizeSelect">
                ...
                <ul class="pagination">
                    ...
                    <li :class="scenePage.currentPage == 1 ? 'disabled' : ''">
                        <a @click="turnToScenePage(scenePage.currentPage - 1)" href="javascript:;">上一页</a>
                    </li>
                    <li>
                        <a href="javascript:;" @click="turnToScenePage(scenePage.currentPage - 3)" v-text="scenePage.currentPage - 3" v-if="scenePage.currentPage - 3 > 0"></a>
                    </li>
                    <li>
                        <a href="javascript:;" @click="turnToScenePage(scenePage.currentPage - 2)" v-text="scenePage.currentPage - 2" v-if="scenePage.currentPage - 2 > 0"></a>
                    </li>
                    <li>
                        <a href="javascript:;" @click="turnToScenePage(scenePage.currentPage - 1)" v-text="scenePage.currentPage - 1" v-if="scenePage.currentPage - 1 > 0"></a>
                    </li>
                    <li class="active">
                        <a href="javascript:;" @click="turnToScenePage(scenePage.currentPage)" v-text="scenePage.currentPage">3</a>
                    </li>
                    <li>
                        <a href="javascript:;" @click="turnToScenePage(scenePage.currentPage + 1)" v-text="scenePage.currentPage + 1" v-if="scenePage.currentPage + 1 <= scenePage.totalPage"></a>
                    </li>
                    <li>
                        <a href="javascript:;" @click="turnToScenePage(scenePage.currentPage + 2)" v-text="scenePage.currentPage + 2" v-if="scenePage.currentPage + 2 <= scenePage.totalPage"></a>
                    </li>
                    <li :class="
                scenePage.currentPage == scenePage.totalPage ? 'disabled' : ''
              ">
                        <a href="javascript:;" @click="turnToScenePage(scenePage.currentPage + 1)">下一页</a>
                    </li>
                    <li :class="
                scenePage.currentPage == scenePage.totalPage ? 'disabled' : ''
              ">
                        <a href="javascript:;" @click="turnToScenePage(scenePage.totalPage)">尾页</a>
                    </li>
                </ul>
                <div class="go">
                    <div :class="isPageNumberError ? 'input-group error' : 'input-group'">
                        <input size="small" class="form-control" type="number" v-model="goToPage" min="1" :max="scenePage.totalPage" />
                ...
                <p class="page-info nowrap">
                    ...
                    <span class="text-primary" v-text="scenePage.totalPage"></span>页，共有
    ...
    <el-dialog title="测试计划管理" :visible.sync="testPlanDialog" width="50%">
        ...
        <el-row>
            <el-pagination @size-change="turnToPage(1)" @current-change="
            (val) => {
              turnToPage(val);
            }
          " :current-page="page.currentPage" :page-sizes="[5, 10, 20, 50]" :page-size="page.pageSize" layout="total, sizes, prev, pager, next, jumper" :total="page.totalPage">
            </el-pagination>
        </el-row>
    </el-dialog>
    <!--新增测试轮次 -->
                ...
                <div class="modal-body">
                    <form class="form-horizontal">
                        <div class="form-group">
                            ...
                            <div class="col-sm-10">
                                <input size="small" type="text" v-model="addRoundRowData.roundName" class="form-control" autocomplete="off" />
                            </div>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-10">
                                <select size="small" class="form-control" v-model="addRoundRowData.recordmanagementflag">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-10">
                                <input size="small" type="text" v-model="addRoundRowData.timeexecutesetting" class="form-control" autocomplete="off" />
                            </div>
                        </div>
                    </form>
                    <el-row type="flex" justify="center" style="margin-top:10px">
                        ...
                        <el-button size="small" type="primary" @click="addTestRound()">
                            确认
                        </el-button>
                    </el-row>
                </div>
    ...
    <el-dialog title="新增测试计划" :visible.sync="addTestPlanDialog" width="50%">
        <div class="modal-body">
            <form class="form-horizontal">
                <div class="form-group">
                    <label for="testplan-name" class="col-sm-2 control-label">测试计划名称</label>
                    <div class="col-sm-10">
                        <input size="small" type="text" v-model="addRowData.nameMedium" class="form-control" autocomplete="off" />
                    </div>
                </div>
                <div class="form-group">
                    <label for="testplan-desc" class="col-sm-2 control-label">测试计划描述</label>
                    <div class="col-sm-10">
                        <input size="small" type="text" v-model="addRowData.descMedium" class="form-control" autocomplete="off" />
                    </div>
                </div>
                <div class="form-group display" v-show="1 === editType">
                    <label class="col-sm-2 control-label">测试阶段</label>
                    <div class="col-sm-10">
                        <select size="small" name="testPhaseId" id="" class="form-control" v-model="addRowData.testPhaseId">
                ...
                <div class="form-group display" v-show="1 === editType">
                    ...
                    <div class="col-sm-10">
                        <select size="small" name="testRoundId" v-model="addRowData.testRoundId" style="width: 80%; height: 34px" filterable placeholder="请选择">
                            <option v-for="testRound in testRoundArray" :key="testRound.id" :label="testRound.name" :value="testRound.id"></option>
                        </select>
                    </div>
                </div>
            </form>
        ...
        <el-row type="flex" justify="center" style="margin-top:10px">
            <el-button size="small" type="primary" id="btn-add" @click="add()">
                确定
            </el-button>
            <el-button size="small" data-dismiss="modal">
                取消
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form class="form-horizontal">
                        <div class="form-group">
                            <label class="col-sm-8 control-label">执行用例出错，请选择是否继续执行，{{
                    interruptTime
                  }}后自动继续执行</label>
                        </div>
                    </form>
                    <div class="modal-footer">
                        <el-button size="small" type="primary" @click="runContinue()">
                            继续执行
                        </el-button>
