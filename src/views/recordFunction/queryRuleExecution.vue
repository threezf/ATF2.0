...
<script>
    ...
    export default {
        ...
        created() {
            ...
            this.params.queryStartTime = this.params.queryEndTime - 48 * 30 * 24 * 60 * 60 * 1000;
        ...
        methods: {
            ...
            timeSlotSelect() {
                ...
                if(this.timeSlot != '左侧日历表选择'){
                   ...
                   if(this.timeSlot === '近一天'){
                       this.params.queryStartTime = this.params.queryEndTime - 1 * 24 * 60 * 60 * 1000;
                   }else if(this.timeSlot === '近一周'){
                       this.params.queryStartTime = this.params.queryEndTime - 7 * 24 * 60 * 60 * 1000;
                   }else{
                       this.params.queryStartTime = this.params.queryEndTime - 30 * 24 * 60 * 60 * 1000;
                   }
                   console.log('今天',today)
                   console.log('开始时间：',this.params.queryStartTime,'结束时间',this.params.queryEndTime);
            ...
            handleRunStatusChange() {
                this.params.runStatus = this.executionStatuses.indexOf(this.selectedExecutionStatus);
...
<style scoped>
    .timeCol,.executionStatus,.toStyle,.timeRange{
        padding-top: 8px;
        color: gray;
        font-size: 16px;
    }
    ...
    .colorSuccess,.colorFail,.colorJump {
        width: 90px;
        height: 50px;
        margin: auto;
    }
