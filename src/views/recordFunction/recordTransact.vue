...
<div class="page-base-inner">
    <el-container>
        <el-header class="headerRow">
            ...
            <el-button type="primary" icon="el-icon-folder-add" size="small" @click="importFunctionButton">批量导入</el-button>
            <el-button type="primary" icon="el-icon-edit-outline" size="small" @click="updateFunctionButton">修改</el-button>
            <br />
        </el-header>
        <el-main class="el-main-base-inner">
            ...
            <el-row>
                ...
                <el-col :span="4">
                    <el-select v-model="ownedSystem" placeholder="所属被测系统" @change="changeAutId">
                        <el-option v-for="(item,key) in autRespDTOList" :key="key" :value="item.nameMedium">{{item.nameMedium}}</el-option>
                    </el-select>
                </el-col>
            </el-row>
            <!--表格-->
            <el-table ref="singleTable" style=" margin-top: 15px" class="table" :data="tableData" :default-sort="{prop:'modifiedTime',order:'descending'}" stripe highlight-current-row border>
                ...
                <el-table-column prop="code" label="编码" align="center">
                    <template slot-scope="scope">
                        <a class="numberA" @click.prevent="toDatail(scope.row.id,scope.row.autId, scope.row.elementRepositoryId)">{{scope.row.code}}</a>
                    </template>
                </el-table-column>
                <el-table-column prop="nameMedium" label="名称" align="center"></el-table-column>
                <el-table-column prop="transType" label="类型" align="center"></el-table-column>
                <el-table-column prop="descShort" label="描述" align="center"></el-table-column>
                <el-table-column prop="createTime" label="创建时间" align="center" :formatter="transTime" sortable></el-table-column>
                <el-table-column prop="modifiedTime" label="修改时间" align="center" :formatter="transTime" sortable></el-table-column>
            </el-table>
            <!--底部换页-->
            <div>
            ...
            <!--新增和修改对话框-->
            <el-dialog width="26%" :title="dialogTitle" :visible.sync="dialogVisible" :before-close="handleBeforeClose">
                <el-form :model="ruleForm" ref="ruleForm" :rules="rules" status-icon>
                    ...
                    <el-form-item class="formItem" label="描述" label-width="110px" prop="descShort">
                        <el-input type="textarea" class="formInput" cols="5" rows="5" v-model="ruleForm.descShort"></el-input>
                    </el-form-item>
                    <hr color="#F5F5F5" />
                    <el-form-item>
            ...
            <!--导入对话框-->
            <el-dialog width="26%" :title="dialogTitle" :before-close="handleBeforeClose" :visible.sync="dialogImportVisible">
                <el-form :action="importURL" enctype="multipart/www-form-data" method="post" id="formData">
                    <el-upload ref="upload" :limit="1" :action="importURL" :auto-upload="false" :file-list="fileList" :on-preview="handlePreview" :on-remove="handleRemove" :on-exceed="handleExceed" :on-change="handleOnChange">
                    ...
                    <el-form-item>
                        <div class="dialogImportBottom">
                            ...
                            <div>
                                ...
                                <el-button size="small" type="warning" plain @click="cancelButton">取消</el-button>
                            </div>
                        </div>
                    </el-form-item>
                </el-form>
...
export default {
    ...
    created() {
        ...
        this.ownedSystem = sessionStorage.getItem("autName");
        this.getAllFunction();
        this.getAllSystem();
    },
    mounted() {},
    computed: {
        ...
        importURL() {
            return "http://140.143.16.21:9090/atfcloud2.0a/transactController/batchImportTransact"; // 上传的URL
    ...
    methods: {
         ...
         */
        // 跳转到详情
        toDatail(id, autId, elementRepositoryId) {
            console.log("recordDetail", id, autId, elementRepositoryId);
            this.$router.push({
         ...
         */
        getAllFunction() {
            console.log("pagedBatchQueryTransact", this.changedParams);
            Request({
