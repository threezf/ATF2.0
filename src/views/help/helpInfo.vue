...
export default {
   ...
   async created() {
        try {
           this.projectList = await getJsonDataApi().then();
            this.getdata()
        }catch (err) {
            console.log(err)
            alert('请求出错')
    ...
    methods: {    
        getdata(){
            ...
            if (this.showByCategory) {
                ...
                const newkey = Object.keys(this.categoryMap).sort()
                var newObj = {}
                for(var i =0; i <newkey.length; i++){
                    newObj[newkey[i]] = this.categoryMap[newkey[i]] 
                }
                this.categoryMap = newObj;
                this.projects = [];

                for (var i =0; i <newkey.length; i++) {
                    let pros = categoryObjMap.get(newkey[i]);
                    for (let pro of pros) {
